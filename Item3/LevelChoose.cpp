...
void LevelChoose::levelChoose(const int &level)
{
	...
	switch (level) {
	case 4:QObject::connect(button[4], SIGNAL(clicked()), this, SLOT(createmap1())); 
	case 3:QObject::connect(button[3], SIGNAL(clicked()), this, SLOT(createmap1()));
	case 2:QObject::connect(button[2], SIGNAL(clicked()), this, SLOT(createmap1()));
